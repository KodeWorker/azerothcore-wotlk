...
| 8341–8540 (no skip-listed IDs in range) | 2026-07-18 | 16 targeted fixes across 16 quest rows | Batch 43. 173 real rows in range; 62 flagged, moderate density (36%) — dominated by a huge AQ War Effort repeatable-donation quest cluster (Copper/Thorium/Silithus Bars, bandages, hides, food) and a smaller "Cenarion Hold Templar/Duke" elite-summon cluster. **The most significant find this batch**: a genuine multi-quest content-swap bug in the Templar/Duke cluster, confirmed via same-row internal inconsistency (each affected row's own `ObjectiveText4` field already had the *correct* turn-in NPC, while `Title`/`Details`/`Objectives` had the wrong one) as well as English cross-checks — quests 8535 ("Hoary Templar") and 8537 ("Crimson Templar") had been mislabeled with `深淵公爵`/`微風公爵` ("Duke of Fathoms"/"Duke of Zephyrs" — titles that belong to a *separate*, not-yet-reached quest pair) instead of `蒼白聖殿騎士`/`赤紅聖殿騎士`; quest 8538 ("The Four Dukes") had been genericized into "4 Abyssal Templars" instead of naming its actual four named Dukes, and its turn-in NPC field said the wrong person entirely — replaced wholesale with wowhead's version, which matches the English source closely on all three fields. A related, much larger naming bug: `喚風者梅恩·長角` is being used as an incorrect stand-in for **at least three different real turn-in NPCs** (Windcaller Proudhorn, Bor Wildmane, Commander Mar'alith) across a name that recurs in **29 quest rows total** — fixed the 5 in-batch instances (8496/8534/8535/8537/8538) after individually confirming each one's real NPC via English and/or the row's own `ObjectiveText4`; the other ~24 occurrences are outside this batch's range and were deliberately **not** blind-swept, since each needs its own correct-NPC determination rather than a single find-replace (flagged prominently for future batches). Two more genuine wrong-name fixes in the same AQ War Effort cluster: `達米`→`妲姆·雙辮`, "**Dame Twinbraid**," quests 8499/8500 (DB had an entirely different, truncated name); and a dropped rank (`布羅恩·石眉`→`中士布羅恩·石眉`, "**Sergeant** Stonebrow," quest 8492 — wowhead's own fetch had the wrong rank here too, "Lieutenant," so this was constructed from the English source directly rather than copied from wowhead). A genuine character-substitution fix (`準將`→`准將`, "Brigadier General" — `准` is the grammatically correct rank-modifier character here, not `準`; corpus already favored the correct form 57:16, swept the 16 wrong instances found in-batch: quests 8371/8372/8399–8403). A genuine mid-name typo (`波尼?碎石`→`波尼·碎石`, quest 8512 — a stray `?` where the middle-dot separator should be; the row's own `ObjectiveText4` already had it right). Confirmed a long list of false positives, several going *against* wowhead on solid English-source or corpus-majority grounds: `在每個戰場上都取得1枚榮譽獎章`/wowhead's explicit 4-battleground list (quest 8367/8371 — English's Objectives is generic "from each battleground," not an itemized list; wowhead reflects a later-revised version of this quest, the same wrong-era-content pattern from batch 37); `萬惡之源`/wowhead's `萬惡根源` ("The Root of All Evil," quest 8481 — both are valid, but DB's phrasing is the more idiomatic/recognizable Chinese rendering of this specific English idiom); `戰歌前鋒`/wowhead's `戰歌偵查騎兵` (12:2 DB majority); `此任務已經絕版` editorial note (quest 8344 — a deliberate, informative DB-added annotation, kept as-is, same treatment as the `(PvP)` suffix precedent from batch 40). Also reconfirmed the now-familiar `$N`/`&lt;name&gt;` wowhead-HTML-entity-leak false positive and the blank-English-source pattern (quests 8469 etc.), plus two count-mismatch-tool false positives worth noting for the pattern: quest 8524/8525's "110%" and quest 8530's "39 companions" are rhetorical/flavor numbers in the Details text, not real quest-completion counts, and don't indicate any actual mismatch. |

| 8541–8740 (excl. 2 skip-listed; 8568, 8571) | 2026-07-21 | 143 targeted fixes across 141 quest rows (130 title/details/objectives + 13 same-row `ObjectiveText4` NPC-name fixes, 3 overlapping) | Batch 44. 198 real rows in range; 177 flagged, by far the highest density yet (89%) — dominated by the AQ epic-armor reputation questline (Vek'nilash/Vek'lor "Conqueror/Oracle/Stormcaller/Vindicator" 4-class armor sets, one quest per set piece, heavy shared boilerplate per NPC/slot) and the AQ War Effort donation-quest cluster continuing from batch 43. **A new wowhead scrape-failure signature**, distinct from the three found in prior batches: a Lunar Festival quest-index placeholder (`一 新年慶典 任務.。` / `巫妖王之怒 任務`) hit 46 of the 198 fetched quests (mostly the AQ Confessor/Templar elite-summon and gong-donation quest clusters) — confirmed and left entirely unchanged, same treatment as the Darkmoon-Faire/WotLK-index patterns. Given the unprecedented density and that a sample of DB's existing text showed literal untranslated English words mid-sentence (quest 8740: `Twilight Marauder Morna`, `5個Twilight Marauders`) alongside a wrong kill count (English/`RequiredNpcOrGoCount` confirm 2, not 5), this batch didn't narrate each fix individually — it bulk-replaced Title/Details/Objectives with wowhead's text for all 131 non-scrape-failure flagged quests (matching the batch-36 wowhead-default policy), then verified via `audit_quest_counts.py` and targeted English cross-checks rather than per-quest narration. Count audit caught 3 pre-existing mismatches, all resolved by the wowhead text itself once applied (quest 8541's "10/10/10" grinding stones, quest 8730's "5 hours" time limit that DB had wrongly rendered as "1.5 hours," quest 8740's "2" marauders) — and 3 new false positives after the pass, all reconfirmed as the established spelled-out-numeral/percentage/word-number audit-tool quirks (quest 8620's `八章` for "8 chapters," quest 8728's "99%" flavor text, quest 8735's Chinese-digit `4` where English spells "four" as a word). **Two confirmed false positives where DB was already more correct than wowhead's own fetch, left unchanged**: quests 8579/8595 title `凡人中的勇士` ("Heroes among Mortals") is a much closer literal match to English "Mortal Champions" than wowhead's `人類勝利者` ("Human Victor," which drops "mortal" for a race-specific "human" not present in the English at all). **Extended the batch-43-flagged `喚風者梅恩·長角` misnamed-NPC bug**: fixed 13 in-batch `ObjectiveText4` occurrences (9× the ring-questline's own already-correct `喚風者耶薩德拉`, confirmed via each row's own `Objectives` field; 3× `喚風者傲角`/Windcaller Proudhorn, confirmed via wowhead+English for the newly-applied Objectives text; 1× quest 8732, an entirely different NPC, `克魯格·碎顱`/Krug Skullsplit, confirmed via English `EndText` and neighboring quest 8731's own wowhead-sourced text) — this confirms the placeholder name stands in for **at least four** different real NPCs, not three as batch 43 found. ~10 more occurrences (quests 8778–8810) remain outside this batch's range, deliberately not swept. **Two tooling fixes, off-cycle**: `diff_quest_text.py` and `audit_quest_counts.py` had hardcoded absolute paths from a different machine/user (`/home/ditcommon/...`) left over from an earlier session — fixed to resolve relative to `~/github/azerothcore-wotlk` (or `$ZHTW_QUEST_SQL`/`$ZHTW_SCRATCH` env overrides) so the toolkit works in this environment; also self-caught and fixed a stdout/stderr buffering artifact in this batch's own diff-generation step that had bled the diff tool's summary line into quest 8731's `Details` field text before the fix was applied — caught via a post-fix verification diff pass (the same "git diff sanity pass before finishing" discipline established in batch 30), not by the initial application. |

| 8741–8940 (excl. 1 skip-listed; 8742) | 2026-07-21 | 111 targeted fixes across 108 quest rows (91 title/details/objectives bulk-replaced + 3 same-row `ObjectiveText4` NPC-name fixes + 17-occurrence name-spelling sweep across 16 rows, some overlapping) | Batch 45. 198 real rows in range; 114 flagged, high density (58%). Continues the AQ War Effort/hive-assault cluster from batch 44, then transitions into a Winter Veil (Christmas) holiday-quest cluster and a Silithus epic-wristguard reward chain. **Confirmed a genuine multi-quest content-swap bug**, same pattern as prior batches: quests 8778–8781/8786/8787/8807 (7 War-Effort donation quests, each meant to deliver different materials to a different NPC — explosives to Nosbin, ore/gems to Lakhsban, armor plates/swords to Jeneela Ironhammer) all had *identical* wrong DB text copied from an unrelated bandage-donation quest; replaced wholesale with wowhead's per-quest-correct text (verified against English `quest_template` for the AQ hive-assault sub-cluster, e.g. quest 8770's DB said "25 Ashi worker scorpions + 40 Ashi male scorpions" while English/`RequiredNpcOrGoCount` confirm "30 Hive'Ashi Defenders" — DB's creature name and count were both wrong). **This fully resolves the `喚風者梅恩·長角` misnamed-NPC backlog from batches 43–44**: the 7 content-swapped quests above plus 3 more same-row `ObjectiveText4` fixes (8782/8808/8810, confirmed `喚風者傲角`) bring the corpus-wide count to **zero remaining occurrences** — see updated Established-terms entry. **7 quests where wowhead has no zhTW translation at all** (raw bracketed English fallback, a new pattern distinct from the generic-placeholder scrape failures): quest 8741 was left unchanged (DB's existing text is coherent and matches the questline, no better source available); the "Just Compensation" reward-upgrade cluster (8931/8932/8934/8937/8938/8939) had 5 of 6 left unchanged since DB's existing text was already coherent and closely matched sibling quests' now-corrected structure, but quest 8931 got a targeted fix (title `小小的補償`→`公平的補償` and item term `野獸追獵者`→`馭獸者`, both matching the corrected sibling pattern applied elsewhere in this same batch, plus dropped an English-unsupported "I promise it'll be better" sentence not present in this quest's actual English source). **A recurring NPC name-spelling split resolved**: `德莉亞娜`→`德莉娜` (Deliana, the Ironforge quest-giver for this reward chain) — corpus was internally split with zero prior majority; wowhead confirmed `德莉娜` unanimously across every real-hit instance in this batch (16 rows swept in-range); ~23 more occurrences outside this batch's range (quest IDs from ~8951 through 10498) remain unswept. Also resolved the matching `野獸追獵者`/`馭獸者` (Beaststalker's) split for the wristguard cluster this batch touched. **Confirmed 3 false positives where DB beat wowhead outright**, a "wowhead shows wrong content" case distinct from the known wrong-era/scrape-failure patterns: the "Dearest Colara"/"Dearest Elenia" love-letter quest trio (8897/8898/8901) — wowhead's fetch returned entirely different, unrelated NPC names (`賈克林·海德`/`克萊拉`/`艾妮亞` vs DB's `亨德瑞克`/`科蘭拉`/`艾琳娜`); English `quest_template` confirms DB's names exactly (`Lieutenant Heldric`, `Colara Dean`, `Elenia Haydon`) — left unchanged. Count audit found 2 remaining flags after the pass, both reconfirmed as the established word-form-number/flavor-text quirks (quest 8898's `12` for spelled-out "twelve," quest 8928's `40` gold price-cap mention, both present in English too just as words/context rather than a real requirement count). |

| 8941–9140 (excl. 8 skip-listed; 8971–8976, 9031, 9051) | 2026-07-21 | 159 targeted fixes across 155 quest rows (139 title/details/objectives bulk-replaced + 20-occurrence name-spelling sweep across 19 rows, some overlapping) | Batch 46. 192 real rows in range; 155 flagged, very high density (81%). Continues the Valthalak's Amulet questline (Blackrock Mountain/Scarlet Crusade) into a huge Naxxramas Silver Hand tier-armor reward chain (Redemption/Crypt Stalker/Frostfire/Plaguehallow/Faith/Earthshatter/Boneweave sets, one quest per class-armor-slot, heavy shared boilerplate). **Confirmed a genuine wowhead-shows-wrong-content false positive**, the same class of issue as the wrong-era-content and scrape-failure patterns but a new variant: quest 9065 ("The Pattymac Special," a real Warlock class quest — "destroy that tower") — wowhead's fetch returned entirely unrelated, garbled placeholder-looking text (`The "Chow" Quest (123)aa` / `Kill Kobold Vermin, 2 of em.`) that doesn't match this quest at all; DB's existing text is a clean, correct match to the English source — left unchanged. **Extended two established name-spelling corrections and settled two new ones**, all confirmed in this same questline this batch, applying wowhead's unanimous-across-every-real-hit spelling over DB's inconsistent originals: `瓦塔拉克`→`瓦薩拉克` (Lord Valthalak, folded into the general bulk pass since nearly every instance had a real wowhead hit), `伯德雷`/`佈德利`→`布德利` (Bodley — DB had *three* different spellings for one NPC), and continued the batch-45 `德莉亞娜`→`德莉娜` (Deliana) and `野獸追獵者`→`馭獸者` (Beaststalker's) sweeps into this batch's own range (20 occurrences across 19 rows, including 6 rows where wowhead had no translation at all so the fix had to be applied directly against DB's own pre-existing shared-boilerplate text rather than sourced from a wowhead diff). Also resurfaced `安泰恩`→`安希恩` (Anthion, established batches 44–45) throughout this same cluster, resolved via the general bulk pass. 15 more wowhead-no-translation (bracketed-English-fallback) quests handled the same way as batch 45 — left unchanged except for the established-term sweep, since DB's existing text for these was already coherent and matched the now-corrected sibling rows' structure. Count audit: 0 mismatches after the pass (only 1 flagged before, quest 9052's blank Objectives now correctly filled with "8" via the bulk replace, matching English exactly). |

| 9141–9340 (excl. 11 skip-listed; 9231, 9284–9286, 9296–9298, 9306–9308, 9316) | 2026-07-21 | 71 targeted fixes across 71 quest rows (68 title/details/objectives bulk-replaced + 3 individually-constructed fixes) | Batch 47. 189 real rows in range; 73 flagged, moderate density (39%). Covers the Ghostlands/Deatholme Blood Elf questline, the Silver Covenant Naxxramas gear-exchange cluster, the Atiesh (Guardian's staff) legendary questline, and a "graveyard rune"/Scourge-invasion investigation cluster. **A genuine content-swap bug found and fixed by direct construction rather than a wowhead diff**: quest 9339 ("A Thief's Reward") had a completely unrelated quest's leftover raw English text sitting in its Objectives field (`You've proven yourself, today, <name>...a fitting gift for a conqueror`, matching neither this quest's own English source nor wowhead, which itself has no zhTW translation for this ID) — confirmed the real English via `quest_template` (`Return the Flame of Stormwind to (NAME).` / `This flame, stolen from the heart of Stormwind itself...`, including a literal `(NAME)` placeholder that's part of the actual English source, not a translation gap) and constructed the correct translation directly. Two more individually-resolved bracket-fallback (no-wowhead-translation) quests: quest 9269, whose DB text was the identical pre-fix shared boilerplate as the confirmed-correct siblings 9270/9271 (Atiesh questline), copied their corrected text directly; quest 9321, whose English title (`Super Healing Potion`) is character-for-character identical to sibling 9336's wowhead-confirmed `極效治療藥水`, applied the same title fix by analogy. A 4th bracket-fallback quest (9168, "Heart of Deatholme") was checked against English and found already correct — left unchanged. **Confirmed a name-spelling correction resurfacing from earlier established rules**: `阿泰絲` (Atiesh) supersedes DB's `埃提耶什` throughout the questline this batch touched, matching wowhead unanimously. Count audit: 0 real mismatches — all 5 remaining flags (quests 9262–9265, 9333) are the established blank-English-source false positive (wowhead's own count value, trusted per prior-batch precedent, just not traceable to the English source's blank field). |

| 9341–9540 (excl. 15 skip-listed; 9384, 9411–9414, 9445, 9458, 9459, 9477–9482, 9507) | 2026-07-21 | 129 targeted fixes across 129 quest rows (127 title/details/objectives bulk-replaced + 2 individually-constructed content-swap fixes) | Batch 48. 183 real rows in range; 145 flagged, very high density (79%). Covers the Hellfire Peninsula/Ghostlands leveling arc almost end-to-end (Thrallmar/Honor Hold/Falcon Watch/Mag'har Post questlines), the Azuremyst Isle elemental-shaman chain (resurfacing the `碧藍崗哨`→`藍色守望`/`阿魯古`-cluster naming issues first found via direct user report before this batch), and the Zangarmarsh/Cenarion Refuge Legion-invasion questline. **A second instance of the exact batch-47 content-swap bug**: quest 9365 ("A Thief's Reward") had the identical wrong leftover raw English text as batch 47's quest 9339 sitting in its Objectives field, confirmed via the identical English `quest_template` row — fixed the same way, by direct construction from the verified English source rather than a wowhead diff (wowhead has no translation for this ID either). **Three genuine count/content bugs caught by `audit_quest_counts.py` and resolved via the bulk pass**: quest 9516 (DB's generic "kill 15 demons in three named locations" replaced with English's actual 6/6/6 breakdown by named demon type); quest 9517 (DB's objectives had silently dropped an entire required item — "15 Warsong timber" — leaving only the "5 satyr hellwood" half); quest 9518 (DB's objectives named completely different, wrong required kills — "Overseer Gorzeeki + any 12 Horde" — versus the real "Overseer + 2 harvesters + 5 lumberjacks + 10 scouts"). All three matched wowhead exactly once fixed. One false positive reconfirmed post-fix: quest 9443's sole remaining "diff" is a pure apostrophe-escaping artifact (`'神聖'` vs `\'神聖\'`) with byte-identical content otherwise — the established notation-artifact exception, not a real disagreement. Count audit: 1 residual flag (quest 9364's English "50" is absent from both DB and wowhead's own zhTW text, which paraphrases the count as "殺夠了"/"kill enough" instead of stating a digit — a legitimate translation choice both sides independently made, not a bug). |

| 9541–9740 (excl. 21 skip-listed; 9577, 9596, 9597, 9599, 9613–9615, 9650–9662, 9679) | 2026-07-21 | 92 targeted fixes across 92 quest rows (bulk-replaced; 1 of the 92, quest 9610, got a further manual count correction) | Batch 49. 179 real rows in range; 101 flagged, moderate-high density (56%). Covers the Draenei elemental-shaman chain (more `碧藍崗哨`→`藍色守望` resurfacing), the Hellfire Peninsula wrap-up (Kargath/Nazzivian satyr cluster), and the Silvermoon Blood Knight order questline (Karazhan/Auchindoun sub-arcs included). **Caught and reverted a wowhead-was-wrong case** during post-fix verification: quest 9610 ("Pool of Tears") — DB's pre-existing text already correctly said "5 Atal'ai Artifacts," matching English `quest_template` exactly, but wowhead's own fetch said "10"; the bulk pass initially took wowhead's number, then `audit_quest_counts.py`'s post-pass run caught the mismatch (EN=5, DB=10) and it was corrected back to 5, keeping wowhead's improved phrasing/NPC-name spelling but restoring the correct count — a reminder that even within a single quest, wowhead's prose can be right while its numbers are wrong (or vice versa), so the count audit has to run *after* a bulk pass, not just before it. 6 wowhead-no-translation quests all checked individually and left unchanged: 5 were already coherent and matched English/sibling context (quest 9695's DB text even preserves a trailing `\r\n` artifact that's present in the *English* `quest_template` source itself, not a DB bug); quest 9583 ("Omar's Test Quest") is a genuine minimal QA/dev quest in the English source too, and DB's terse translation already matches it exactly. One notation-artifact false positive reconfirmed (quest 9561, same apostrophe-escaping class as batch 48's quest 9443). Count audit: 0 real mismatches after the 9610 correction. |

| 9741–9940 (excl. 1 pre-existing skip-listed 9745; 1 more removed mid-batch, 9750) | 2026-07-21 | 165 targeted fixes across 165 quest rows (bulk-replaced, 28 with the `objectives` field deliberately withheld) + 1 quest removed to skip-list | Batch 50. 197 real rows in range; 176 flagged, extremely high density (89%). Covers the Zangarmarsh Telredor/Cenarion Refuge wrap-up, the entire Karazhan key-fragment questline (Master's Terrace/Violet Eye), and the huge Nagrand "Throne of the Elements"/Hemet Nesingwary hunting/Garadar-Telaar war-effort cluster. **The most significant finding this pass since the count-audit tool was introduced**: proactively cross-checked wowhead's own fetched numbers against the true English source *before* applying anything (not just DB-vs-English after the fact) and found wowhead's kill/collect counts were wrong in **28 separate quests**, almost entirely concentrated in the Nagrand "Throne of the Elements" elemental-lord dailies and the Nesingwary hunting dailies (e.g. quest 9789: English/DB both correctly say "12 Clefthoof," wowhead said "30"; quest 9800: English/DB say "8," wowhead said "15"; pattern repeats across 9804/9810/9819/9821/9850/9854/9855/9857/9858/9862/9863/9865/9878/9906/9907/9916/9921/9922/9925/9927/9928/9931/9935/9936/9939/9940 — DB's pre-existing counts were correct in every single one). Applied wowhead's Title/Details fixes normally for these 28 (those were fine) but deliberately withheld the `Objectives` field, keeping DB's already-correct count text — confirmed by a second post-pass count audit showing 0 real mismatches. This changes the working assumption from prior batches (spot-check counts after a bulk pass) to proactively diffing wowhead's own numbers against English *before* applying, at least for any batch with a repeatable-daily/hunting-quest cluster, since wowhead's count errors cluster heavily in exactly that content type. **One quest moved to skip-list**: quest 9750, whose English title is literally `UNUSED Urgent Delivery` with blank Objectives/Details in the English source itself — confirmed unused/cut content, `INSERT` removed (`DELETE` kept, matching the batch-39/40 precedent). **A second internal-flag quest found and handled without skip-listing**: quest 9926 (`FLAG Shadow Council/Warmaul Questline`, English Objectives/Details/EndText all blank) — wowhead itself returned the raw, unwrapped English title (not the usual `[bracketed]` no-translation signature this pass's parser watches for, a gap worth noting for future batches), which would have regressed DB's existing partial Chinese translation of the flag name back to raw English if blindly applied — excluded just the `title` field from the bulk pass instead, since the quest is otherwise invisible to players and skip-listing felt like the wrong tool for a row that already has a real (if imperfect) translation attempt in the DB. Also confirmed two dev-marker "Turnin Cap" quests (9880/9881, 9908/9909) are genuine functioning quests with untranslatable Blizzard-internal titles — left unchanged since their meaningful field (`CompletedText`, the actual turn-in text) was already correct. Count audit: 1 residual false positive (quest 9868's "3" appears in DB as the Chinese character `三`, not a matching Arabic digit — the established character-numeral audit-tool quirk). |
...
After batch 50, **5,146 verified**, **3,717 remaining** — roughly 19 more
...
Resume from quest ID 8541 (batch 44, target range roughly 8541–8740) following the same
two-phase methodology, skipping any ID present in `skip-list.tsv`. 5,054 quest IDs remain
after batch 43 (see Total scope note above).

**Addendum from batch 43**: this batch's range likely borders the *actual* "Duke of Fathoms"/
"Duke of Zephyrs"/etc. quest pair that quests 8535/8537 had wrongly borrowed titles from — check
quest IDs just past 8540 for a `深淵公爵`/`微風公爵`-titled pair and verify they're not
*themselves* mislabeled with Templar names in the opposite direction (a mirrored swap). Also
continue chipping away at the `喚風者梅恩·長角` wrong-NPC-substitution backlog (~24 remaining
occurrences, see the new Unresolved-items entry) as each affected quest ID is reached — verify
individually via English or the row's own `ObjectiveText4`, don't blind-sweep.
