...
| 14141–14340 (4 pre-existing skip-listed IDs: 14147, 14148, 14149, 14150) | 2026-07-22 | 1 targeted fix (single-row proper-noun correction) + 1 new skip-list addition | Batch 72. Extremely sparse range (162/196 IDs not in DB, almost the entire back half of the range) — an Argent Tournament joust/mail-kickoff cluster plus a "Call to Arms: Isle of Conquest" battleground-invite quest. **1 scoped proper-noun fix, not a bulk replace** (quest 14163): the bulk of the diff was pure stylistic paraphrase (not content errors, so left DB's own wording), and the Title's `戰鬥的召喚`("Call to Arms") vs wowhead's one-off `戰鬥的號角` was correctly kept as DB's own overwhelming 26:1 corpus convention for this recurring templated battleground-quest phrase outranking a single fetch. The one genuine fix was the Kor'kron guard-unit name in Details (`庫卡隆`→`柯爾克隆`), which lacks DBC coverage but matched both wowhead's rendering and the corpus's own already-existing (if narrow, 9:7) majority — scoped to just this row's occurrence, not swept, given the weak evidence. **1 more instance of the wowhead-tagline-bleed-into-Title bug found and skip-listed**: quest 14200, English `LogTitle` "Kickoff Mail Marker" — matches the established internal-tracker Marker pattern exactly. Proactive WH-vs-English count check: 0 candidates. Count audit: 0 mismatches. Linter and row-count/duplicate check clean; `git diff` sanity pass confirmed exactly 1 changed row (14163) — this batch's diff base is the fresh commit from batches 63-71, pushed to `origin/feat/zhTW-translation` between batches. |
| 14341–14540 (7 pre-existing skip-listed IDs: 14350, 14351, 14352, 14353, 14418, 14420, 14439) | 2026-07-22 | 1 targeted fix (genuine count bug) | Batch 73. Extremely sparse range (182/193 IDs not in DB, the sparsest batch yet) — a Mulgore/Thunder Bluff cluster. All 10 present-in-DB rows that flagged were clean bracket-fallback (0 scrape-fail, 0 real). **1 genuine count bug caught by the proactive audit, not the text diff** (quest 14438, "Sharing the Land"): DB's Objectives said `殺死15個白鬃豺狼人`("kill 15 Palemane Gnolls"), but tracing the actual mechanic fields showed only 2 of the 3 creature types named in the English flavor text are real tracked requirements — `RequiredNpcOrGo1`=2950 (Palemane Skinner) count 8, `RequiredNpcOrGo2`=2951 (Palemane Poacher) count 5, while the flavor text's "10 Palemane Tanners" corresponds to no active requirement at all (`RequiredNpcOrGo3`=0). True total is 8+5=13, not 15; fixed. **New count-audit false-positive subclass identified**: when English flavor text names a sub-species breakdown where one type isn't actually tracked, the correct total won't numerically match any of the flavor text's individual digits or their naive sum — verify via the actual `RequiredNpcOrGo`/`RequiredNpcOrGoCount` entry-ID mapping, not just digit-matching, whenever the flavor text lists more sub-types than the required-count fields have non-zero entries for. Proactive WH-vs-English count check: 1 candidate (14438, above, confirmed genuine and fixed). Count audit re-run post-fix: 1 residual flag, the expected new false-positive class described above (0 real mismatches). Linter and row-count/duplicate check clean; `git diff` sanity pass confirmed exactly 1 changed row (14438), no unscoped bleed. |
| 14541–14740 (no skip-listed IDs) | 2026-07-22 | 0 targeted fixes — entire range absent from the DB | Batch 74. **The sparsest batch yet: all 200 IDs are absent not just from `quest_template_locale` but from the base English `quest_template` table entirely** — confirmed directly (0/200 present in either table), not a translation gap at all, just an unused ID range in this content patch's quest numbering. Nothing to fetch, diff, or verify. Verified/remaining tallies unchanged from batch 73 since no rows exist to add to either count. |
| 20438–24923 (9 pre-existing skip-listed IDs in range) — targeted present-ID clusters, not a blind sequential sweep | 2026-07-22 | 17 targeted fixes across 17 quest rows (13 real content bugs + 4 corpus-wide sweep of an already-known duplicate) | Batch 75. **Major methodology discovery**: mapped the entire remaining tail of the ID space (14741–26034) and found it's almost entirely empty — only 264 quest rows exist in that whole 11,294-ID span, clustered into 5 small groups (20438-20439, 24216-24227, 24426-24923, 25055-25500, 26012-26034). Switched from blind sequential 200-ID batches to fetching only the actual present-ID clusters directly — this batch covers clusters 1-3 (208 raw IDs, 199 after skip-list) in one pass, something that would have taken ~28 wasted empty batches under the old pacing. The Icecrown Citadel Shadowmourne-legendary-weapon questline and the "Call to Arms: Warsong Gulch/Arathi Basin" battleground-invite cluster. **3 genuine count/content bugs found via the proactive audit, not the text diff**: quest 24547 ("A Feast of Souls," the famous "kill 1000 Scourge with Shadow's Edge" quest) had DB saying `殺死50名`(50) instead of the correct `1000`(all `RequiredNpcOrGoCount` fields are 0, so English's literal "1000" is the only source, and this is well-documented as accurate to the real quest); quests 24498/24507 (mirror Alliance/Horde "Free 15 Slaves" quests) had DB saying `6個`/`5個 礦場營地`("mine camps," an invented unit) instead of `15名...奴隸`("15 slaves," matching English exactly and each row's own already-correct `CompletedText` field, which said "slaves" all along — a same-row internal-consistency tell). **A cluster of raw, entirely untranslated English text found live in Objectives across 11 rows** (a new variant of the batch-64/66/69 "Objectives repurposed from a different English field" pattern, but this time never translated at all): quest 24743 ("Shadow's Edge," accepting the DK weapon) and quest 24815 ("Choose Your Path," the ring-upgrade dialogue) each fixed individually — 24815 had a real wowhead translation to use (token-artifact-corrected), 24743 had none anywhere on wowhead's page, so constructed directly from the `quest_offer_reward.RewardText` source. A 10-row "A Change of Heart" duplicate (`You seek to change the path you walk, $n?`) was fixed with one direct-constructed translation swept across all 10 occurrences at once — 6 inside this batch's own clusters (24819/24820/24821/24822/24839/24844) and 4 more found via corpus-wide grep entirely outside this batch's range (25246-25249, in the not-yet-reached cluster 4). **Confirmed 2 apparent disagreements as non-bugs**: the "Call to Arms" title's `戰鬥的召喚`(DB, established majority) vs `戰鬥的號角`(wowhead, one-off) — same ruling as batch 72's 14163, kept DB's convention, only stylistic paraphrase elsewhere in these rows, no bulk replace; and the "Purification of Quel'Delar" cluster's Tempered/Tainted item-name split (24564/24594/24598) — verified each ID's own English source independently confirms its own DB wording, a legitimate multi-stage quest-chain variance, not an inconsistency. Proactive WH-vs-English count check: 3 candidates, all confirmed genuine and fixed (24547, 24498, 24507). Count audit re-run post-fix: 0 mismatches. Linter and row-count/duplicate check clean; `git diff` sanity pass confirmed exactly 19 cumulative uncommitted rows changed (14163 + 14438 from batches 72-73, plus this batch's 17), all accounted for. |
| 24924–26034 (1 pre-existing skip-listed ID in range: 25092) — final present-ID clusters, completing the pass | 2026-07-22 | 11 targeted fixes across 11 quest rows (full construction of a previously entirely-untranslated cluster) | Batch 76 (final batch). Re-verified the batch-75 mapping directly against the whole `quest_template` table: confirmed 26034 is the absolute maximum quest ID in the entire base game data, with 0 IDs beyond it — this is conclusively the last batch of the pass. Covered the two remaining clusters (25055-25500, 26012-26034), a Cataclysm-era "Elemental Invasion"/Twilight's Hammer pre-event questline cluster (Stormwind and Orgrimmar mirror pair) plus the tail-end Icecrown/Ruby Sanctum content. **Found and fully constructed 11 quest rows that had never been translated at all** — DB's own text held nothing but the bogus wowhead site-tagline Title with blank Details/Objectives, while several *sibling* quests in the exact same Twilight's Hammer cult questline already had complete, well-formed existing translations elsewhere in the corpus. Rather than leave these open, constructed full Title/Details/Objectives translations directly from the English source, grounding every proper noun in either an already-established corpus term (Thrall `索爾`, King Varian Wrynn `瓦里安·烏瑞恩國王`, Grommash Hold `格羅瑪什要塞`, Valley of Wisdom `智慧谷`, Elwynn Forest `艾爾文森林`, Razor Hill `剃刀嶺`, Stormwind Keep `暴風城堡`, Blood Guard Torek `血衛士托雷克`, Captain Anton `安東上尉`, Mirror Lake Orchard `明鏡湖果園`, the four named cultist NPCs) or a same-row/same-cluster pre-existing field: Cho'Gall's name was initially decided as `喬迦爾` (matching two pre-existing `CompletedText` fields already sitting in this exact quest cluster) over a competing `丘加利` found on wowhead's own dedicated NPC page (`wowhead.com/tw/npc=43324`) — reasoning that in-cluster same-corpus evidence should outrank a generic tier-3 NPC-page fetch. **User-corrected same-day**: `丘加利` is correct, matching the dedicated NPC page after all — swept all 4 occurrences (the 2 pre-existing `CompletedText` fields plus the 2 just-constructed `Details` fields). Lesson: pre-existing same-cluster text is not automatically a stronger signal than a dedicated NPC page just because it's "more local" — the OpenCC-conversion caution (same-corpus consistency can just mean the same error propagated) applies even within a small cluster, not only at full-corpus scale. Don't repeat this override pattern without much stronger justification than proximity alone. Quest 25414's own pre-existing `ObjectiveText`/`CompletedText` fields (already using `異教徒X` for the four cultist names) were followed exactly for same-row consistency, overriding an initially-planned `教徒X` phrasing drawn from a *different* row's convention. This construction pass also resolved 2 of the 3 residual count-audit flags automatically (25348/25418's "defeat 5" objectives, now correctly stated). **1 genuine count fix confirmed via the proactive audit** (quest 25055, "Elemental Research" — its own English source is a `[ph]` placeholder stub, matching the already-skip-listed sibling 25092's documented issue) was investigated but left as-is: DB's existing Chinese content is complete and plausible, with no way to verify the flagged `20` against genuinely unfinished English — noted, not changed, distinct from the 11 constructed rows since this one already has real content. Count audit re-run post-fix: 1 residual flag (25055, the unverifiable case above), 0 real mismatches. Linter and row-count/duplicate check clean; `git diff` sanity pass confirmed exactly the intended 11 rows changed (the 12th "fix" was the count-audit investigation that resulted in no edit). **Pass-completion verification**: directly computed that all 8,858 `quest_template_locale` rows are accounted for — 355 are permanently skip-listed (by design, never subject to verification) and the remaining 8,503 have now all been reviewed across all 76 batches, with the tail (14741-26034) specifically confirmed complete by this batch and batch 75's direct row-presence queries. **This closes out the entire content-accuracy verification pass.** |
...
**PASS COMPLETE as of batch 76 (2026-07-22).** Final count, computed directly rather than by
incremental tracking: `quest_template_locale` holds 8,858 rows (8,855 unique IDs + 3 duplicate-ID
extra rows at 1241/1250/1264, a separate pre-existing issue, see Unresolved items). **355 of
these are permanently skip-listed** (deliberately excluded from verification by design — dev/test
content, internal trackers, confirmed duplicates, etc. — see `skip-list.tsv`) and **will never be
"verified"; this is not remaining work**. The other **8,503 rows have now all been reviewed**
across all 76 batches, IDs 1 through 26034 (the confirmed maximum quest ID in the entire base
game data — directly verified batch 76, 0 IDs exist beyond it). The old incrementally-tracked
"X verified / Y remaining" figures in each batch entry above undercounted slightly over the
pass's length (arithmetic drift from 76 rounds of manual updates, not a real gap) — this final
tally supersedes them. **If resuming this pass in the future** (e.g., after a game-content or
translation-source update), don't restart sequential batching — first re-run the query in the
Next batch note below to check whether new rows have appeared.
...
| Doctor Theolen Krastinov | `Name`=`瑟爾林·卡斯迪諾夫教授`, quest-title epithet=`『屠夫』` (RESOLVED 2026-07-25, reversing the earlier no-epithet override) | Originally treated as a "later-expansion official Chinese localization dropped the epithet" preference call, batch-28's `『屠夫』` addition reverted same-day. **Re-examined 2026-07-25**: the base `creature_template_locale.sql` dump (immutable, not the `2026_03_13_04.sql` migration file) has a *separate* `Title` column for entry 11261 that was never touched by that migration — `Name`=`瑟爾林·卡斯迪諾夫教授`, `Title`=`屠夫` — and every other locale (zhCN, deDE, esES/esMX, frFR, koKR, ruRU) confirms the identical Name/Title split (zhCN's own `Title` is also literally `屠夫`). So `屠夫` isn't a stale WotLK-only or later-expansion-only reading competing with `教授` — it's a still-current, multi-locale-corroborated field living in a different column than the one the original review checked. Confirmed independently via `wowhead.com/wotlk/tw/quest=5382`, whose quest title is `『屠夫』瑟爾林-卡斯迪諾夫教授` (matches, modulo a `-`/`·` separator rendering quirk — kept `·` for corpus consistency, see Unresolved items). This project's own English `quest_template.sql` for quest 5382 also independently confirms: Title = `"Doctor Theolen Krastinov, the Butcher"`. Fixed quest 5382's zhTW Title to `『屠夫』瑟爾林·卡斯迪諾夫教授`. `creature_template_locale.Name` itself (教授, no epithet) was correct all along and is unchanged — only the quest-title epithet was missing. See `user-overrides.md` for the full override-reversal writeup. |
...
**This pass is complete — there is no next batch under the old numbering.** All non-skip-listed
`quest_template_locale` rows (IDs 1–26034, the full range that exists in the base game data)
have been reviewed. If this pass is ever resumed (new content added, a translation-source
refresh, or a decision to revisit the skip-list), start by re-running this query to find what's
actually new or changed, rather than assuming sequential-batch numbering still applies:

```python
# find any quest_template_locale IDs not yet covered by a completed-ranges entry above,
# or any *new* IDs added to quest_template since this pass's last run
rows = extract_inserts(TARGET, "quest_template_locale")
current_ids = set(int(unquote(r[0])) for r in rows)
# compare against the ID set this pass actually covered (all of 1-26034 minus skip-list,
# as of 2026-07-22) to find anything genuinely new
```
