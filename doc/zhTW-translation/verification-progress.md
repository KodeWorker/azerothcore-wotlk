...
After batch 75, **8,654 verified**, **204 remaining**. **Batch 74's mapping found the entire
tail (14741–26034) holds only 264 quest rows total, clustered into 5 small groups** — see the
Next batch note below for the exact remaining IDs. The old "~200-ID sequential batch" pacing
no longer applies; the rest of this pass is roughly 1 more targeted batch covering the two
remaining clusters (25055-25500, 26012-26034), not a fixed ID-count estimate.
...
**IDs 14741-24923 are fully done** (batches 74-75 confirmed/covered them, including the huge
14741-20437 and 20440-24215 empty gaps — see the batch-74/75 entries above). Resume batch 76
targeting the two remaining present-ID clusters directly (build the fetch list from
`quest_template` IDs in these ranges, not a blind sequential scan): **25055-25500** (53 IDs)
and **26012-26034** (3 IDs), each minus any `skip-list.tsv` entries. This is expected to be the
final batch of the pass — after it, re-verify the "no present IDs beyond 26034" assumption
(re-run the batch-75 cluster-mapping query) before declaring the pass complete. See the
