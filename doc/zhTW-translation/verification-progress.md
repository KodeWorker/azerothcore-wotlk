...
After batch 45, **4,206 verified**, **4,658 remaining** — roughly 23 more
...
- **`德莉亞娜`→`德莉娜` (Deliana, Ironforge quest-giver, "Just Compensation"/wristguard reward
  chain, found batch 45)** — corpus had zero prior majority either way; wowhead confirmed
  `德莉娜` unanimously across every real-hit instance. 16 rows swept within batch 45's own range
  (8905–8937); ~23 more occurrences remain unswept, roughly quest IDs 8951 through 10498 —
  check each cluster as it's reached, this is a single well-confirmed proper noun so a same-NPC
  sweep is safe once in range, just not yet verified this pass has actually reached those rows.
- **`野獸追獵者`→`馭獸者` (Beaststalker's [item], found batch 45)** — corpus was split with no
  majority; wowhead confirmed `馭獸者` in every real-hit instance. Fixed within batch 45's own
  range only (quests 8906/8914/8931); other corpus occurrences unverified.
